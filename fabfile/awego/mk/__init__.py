...
def keys(keyname="", inregion=""):  #{{{
	with settings(warn_only=True), hide('stdout', 'stderr', 'running', 'warnings'):
		if not keyname:
			keyname = raw_input("Enter a name for the key pair:")
		if not inregion:
			inregion = raw_input("In which region should the key pair belong? :")
		b = BoCoVpc(inregion)
		try:
			kp = b.bc.create_key_pair(keyname) if b.bc else ""
			print(kp.material) if kp else red("Error creating key pair")
		except EC2ResponseError, e:
			print(red(str(e.message)))
  #}}}
@task
def vpc():#{{{
	with settings(warn_only=True), hide('stdout', 'stderr', 'running', 'warnings'):
